...
//buttons
		/*	int buttons_fd;
			...
			for (;;) {
				...
				for (k = 0, count_of_changed_key = 0; k < sizeof buttons / sizeof buttons[0]; k++) {
					if (buttons[k] != current_buttons[k]) {
						buttons[k] = current_buttons[k];
						printf("%skey %d is %s", count_of_changed_key? ", ": "", k+1, buttons[k] == '0' ? "up" : "down");
	*/
					//*return key*//
/*
						(*shmPtr1)=k+1;
...
main(int argc, char *argv[])
{
	...
	if ( screen == NULL ) {
		fprintf(stderr, "Couldn't set 800x480 video mode: %s\n",
							SDL_GetError());
		exit(2);
	}
